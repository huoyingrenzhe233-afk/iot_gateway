...
字段:`when` = 触发条件(传感器 id + 比较符 + 阈值);`then` = 动作(执行器 id + 字段 + 值);`enabled` = 是否启用;`cooldown` = 冷却时间(秒,可选,0=禁用)。
...
>
> 🛡️ **防震荡三件套**(暗光开灯这类"开/关成对规则"的必修课):① **回滞带**——开灯阈值与关灯阈值必须错开(例:开 `<1500`、关 `>=2300`,中间是死区,读数在死区内灯保持现状);② **cooldown**——规则 yaml 里加 `cooldown: 15`,触发后 15 秒内该规则休眠,即使读数反复穿越阈值也不会快速来回下发;③ **物理隔离**——LED 的光漏回光敏传感器会抬高读数形成反馈环,若遮光开灯后读数仍超过关灯阈值,说明漏光太强,需调整遮挡方式或传感器位置。
