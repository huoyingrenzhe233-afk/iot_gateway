...
**💡 规则引擎 cooldown + LED 规则回滞(2026-08-14,用户真机反馈"暗光开灯灯不亮")**:用户把暗光开灯阈值改成 2000,遮光后读数 ~1000,出现"服务端快速下发开关命令、灯好像没亮"。真机排查(gateway.log 实锤):14:37:22-56 期间 bright_led_off/dark_led_on 每 1~4 秒交替触发各发 led_on:0/1——根因 = ① 开/关两条规则阈值重合(2000/2000)零死区,读数在阈值附近抖动即来回触发;② LED 亮起后漏光抬升光敏读数形成物理反馈环(实测:无遮挡读数 ~2450,遮光 ~1000;LED 对读数本身影响约 ±20,但遮光时漏光可能显著)。灯其实在闪(亮 2s 灭 2s),不是没亮。修复:① 规则引擎新增可选 `cooldown`(秒)字段——fire 后 N 秒内该规则休眠(rule_engine.h/.cpp,parse/evaluate/to_json_list 同步);② LED 两条规则改回滞带(开 `<1500`、关 `>=2300`,死区 1500~2300)+ cooldown:15(config/rules/rules.yaml,注释写了防震荡三件套);③ API 文档 §9 补 cooldown 字段与防震荡说明。板上实测:触发→冷却期两次穿越阈值均被抑制→冷却过期后新边沿正常再触发,全链路通过。⚠️ 若遮光开灯后读数仍 >= 2300(漏光太强),需要物理上让 LED 与光敏互相遮光,规则救不了。另:单片机又一度 WiFi 掉线(上报停止),测试用 mosquitto_pub 模拟完成。
